        ...
        async def wrapper(*args, **kwargs):
            try:
                ...
                if "not found" in error_msg and not_found_message:
                    ...
                    raise HTTPException(status_code=409, detail=f"Resource already exists")
                else:
                    # Generic server error
                    error_detail = default_error_message or f"Failed to {operation_name}"
...
class APIErrorHandler:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if "not found" in error_msg and self.not_found_message:
            ...
            # Generic server error
            error_detail = self.default_error_message or f"Failed to {self.operation_name}"
            raise HTTPException(status_code=500, detail=error_detail)