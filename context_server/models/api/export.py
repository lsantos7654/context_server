...
class ContextExport(BaseModel):
    ...
    schema_version: str = Field("1.0", description="Export schema version for compatibility")
    context: dict[str, Any] = Field(..., description="Context metadata and configuration")
    documents: list[dict[str, Any]] = Field(..., description="All documents with metadata")
    chunks: list[dict[str, Any]] = Field(..., description="All chunks with embeddings")
    code_snippets: list[dict[str, Any]] = Field(..., description="All code snippets with embeddings")
    ...
    total_code_snippets: int = Field(..., description="Total number of code snippets exported")
...
class ContextImportRequest(BaseModel):
    ...
    context_data: ContextExport = Field(..., description="Context export data to import")
    overwrite_existing: bool = Field(
        False, 
        description="Whether to overwrite existing context with same name"
    )
...
class ContextImportResponse(BaseModel):
    ...
    imported_code_snippets: int = Field(..., description="Number of code snippets imported")
...
class ContextMergeResponse(BaseModel):
    ...
    merged_documents: int = Field(..., description="Number of documents in merged context")
    merged_chunks: int = Field(..., description="Number of chunks in merged context")
    merged_code_snippets: int = Field(..., description="Number of code snippets in merged context")
    source_contexts_processed: int = Field(..., description="Number of source contexts processed")
    message: str = Field(..., description="Human-readable status message")
