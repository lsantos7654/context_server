...
class HierarchicalGraphBuilder:
    ...
    def __init__(self, database_manager: EnhancedDatabaseManager):
        ...
        self.code_patterns = {
            'python': {
                ...
                'imports': re.compile(r'(?:from\s+\S+\s+)?import\s+([^#\n]+)', re.MULTILINE),
            },
            'rust': {
                'functions': re.compile(r'fn\s+(\w+)\s*(?:<[^>]*>)?\s*\([^)]*\)', re.MULTILINE),
            ...
            'javascript': {
                'functions': re.compile(r'function\s+(\w+)\s*\([^)]*\)|(\w+)\s*=\s*(?:async\s+)?\([^)]*\)\s*=>', re.MULTILINE),
    ...
    async def initialize_schema(self):
        ...
        try:
            ...
            schema_file = "/Users/santos/projects/context_server/hierarchical_graph_schema.sql"
    ...
    async def build_document_hierarchy(
        ...
        try:
            ...
            result = {
                ...
                'total_relationships': len(containment_rels) + len(semantic_rels) + len(impl_rels)
            }
    ...
    async def _create_document_node(
        ...
        node = GraphNode(
            ...
            depth_level=0,
            primary_language=content_analysis.primary_language if content_analysis else None,
            content_type=content_analysis.content_type if content_analysis else 'general',
        ...
        for pattern in code_block_patterns:
            for match in pattern.finditer(content):
                ...
                node = GraphNode(
                    ...
                    parent_node_id=parent_node_id,
                    depth_level=2,
                    primary_language=language or self._detect_chunk_language(code_content),
                    content_type='code',
                ...
                await self._store_node(node)
                await self._store_code_block(node, chunk_data.get('id'), match.start(), match.end())
    ...
    async def _build_containment_relationships(
        ...
        for code_block in code_block_nodes:
            parent_chunk = next(
                (c for c in chunk_nodes if c.chunk_id == code_block.chunk_id), 
                None
            )
            if parent_chunk:
                rel = GraphRelationship(
    ...
    async def _build_semantic_relationships(
        ...
        for i, node1 in enumerate(all_nodes):
            for node2 in all_nodes[i+1:]:
                ...
                if (node1.parent_node_id == node2.id or 
                    node2.parent_node_id == node1.id):
                    continue
    ...
    def _analyze_code_structure(self, code_content: str, language: Optional[str]) -> Dict[str, Any]:
    ...
    def _calculate_semantic_similarity(self, node1: GraphNode, node2: GraphNode) -> float:
    ...
    def _calculate_explanation_score(self, text_node: GraphNode, code_node: GraphNode) -> float:
        ...
        if code_node.primary_language and code_node.primary_language.lower() in text_content:
            score += 0.1
        
        # Check for common tutorial/explanation words near code references
        explanation_words = ['example', 'function', 'method', 'class', 'implement', 'code', 'snippet']
    ...
    async def _store_node(self, node: GraphNode):
        ...
        async with self.db_manager.pool.acquire() as conn:
            await conn.execute(
                ...
                """,
                node.id, node.node_type, node.document_id, node.chunk_id, 
                node.title, node.content, node.summary, node.depth_level,
                node.parent_node_id, node.primary_language, node.content_type,
                node.complexity_score, node.quality_score,
                json.dumps(node.key_concepts), json.dumps(node.extracted_keywords),
    ...
    async def _store_relationship(self, relationship: GraphRelationship):
        ...
        async with self.db_manager.pool.acquire() as conn:
            await conn.execute(
                ...
                relationship.relationship_type, relationship.strength, relationship.confidence,
                relationship.discovery_method, json.dumps(relationship.supporting_evidence),
    ...
    async def _store_code_block(
        ...
        async with self.db_manager.pool.acquire() as conn:
            await conn.execute(
                ...
                """,
                node.id, chunk_id, node.content, node.primary_language,
                ...
                start_pos, end_pos, node.complexity_score
            )
    ...
    async def get_node_hierarchy(self, document_id: str) -> Dict[str, Any]:
        ...
        async with self.db_manager.pool.acquire() as conn:
            ...
            children = await conn.fetch(
                "SELECT * FROM get_node_children($1)",
                doc_node['id']
            )
