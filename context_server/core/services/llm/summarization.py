...
Summary: [your summary here]"""
    ...
    async def generate_title_and_summary(
        ...
        if not self.client:
            logger.warning(
                "OpenAI client not configured, skipping title and summary generation"
            )
            return None, None
        ...
        try:
            ...
            logger.debug(
                f"Generated title: {title}, summary: {len(summary) if summary else 0} chars"
            )
    ...
    async def summarize_batch(
        self, texts: list[str], timeout: float = 120.0, max_concurrent: int = 3
    ) -> list[str | None]:
        ...
        try:
            ...
            tasks = [_summarize_with_semaphore(text, i) for i, text in enumerate(texts)]
            ...
            logger.info(
                f"Batch summarization completed: {successful_count}/{len(texts)} successful"
            )
            return summaries
    ...
    async def generate_titles_and_summaries_batch(
        ...
        if not self.client:
            logger.warning(
                "OpenAI client not configured, skipping batch title and summary generation"
            )
        ...
        async def _generate_with_semaphore(
            ...
            async with semaphore:
                try:
                    ...
                    logger.warning(
                        f"Failed to generate title and summary for chunk {index}: {e}"
                    )
                    return index, (None, None)

        try:
            # Execute all generations concurrently with timeout
            tasks = [_generate_with_semaphore(text, i) for i, text in enumerate(texts)]
            ...
            logger.info(
                f"Generated titles and summaries for {successful_count}/{len(texts)} chunks"
            )
            return title_summaries

        except asyncio.TimeoutError:
            logger.error(
                f"Batch title and summary generation timed out after {timeout}s"
            )
            return [(None, None)] * len(texts)
