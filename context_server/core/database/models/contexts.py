        ...
        async with self.pool.acquire() as conn:
            async with conn.transaction():
                ...
                existing_context = await conn.fetchrow(
                    "SELECT id FROM contexts WHERE name = $1", context_data["name"]
                )

                if existing_context and not overwrite_existing:
                    raise ValueError(
                        f"Context '{context_data['name']}' already exists. Use overwrite_existing=True to replace it."
                    )

                # Delete existing context if overwriting
                if existing_context and overwrite_existing:
                    await conn.execute(
                        "DELETE FROM contexts WHERE id = $1", existing_context["id"]
                    )
                ...
                for chunk_data in chunks_data:
                    ...
                    if chunk_data.get("text_embedding"):
                        text_embedding_str = convert_embedding_to_postgres(
                            chunk_data["text_embedding"]
                        )

                    if chunk_data.get("code_embedding"):
                        code_embedding_str = convert_embedding_to_postgres(
                            chunk_data["code_embedding"]
                        )
                ...
                for snippet_data in code_snippets_data:
                    ...
                    if snippet_data.get("embedding"):
                        embedding_str = convert_embedding_to_postgres(
                            snippet_data["embedding"]
                        )
                    ...
                    await conn.execute(
                        ...
                        snippet_data.get(
                            "preview", snippet_data.get("summary", "")
        ...
        async with self.pool.acquire() as conn:
            async with conn.transaction():
                ...
                if not target_context:
                    raise ValueError(
                        f"Target context with ID {target_context_id} not found"
                    )
