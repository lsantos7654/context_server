        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    echo_error(f"Failed to list jobs: {response.status_code} - {response.text}")
                        ...
                        async with httpx.AsyncClient() as client:
                            ...
                            if response.status_code == 200:
                                ...
                                if phase == "extracting":
                                    ...
                                    description = f"Found {pages} pages, processing content..."
                                ...
                                echo_warning(f"Failed to get job status: {response.status_code}")
                ...
                async with httpx.AsyncClient() as client:
                    ...
                    if response.status_code == 200:
                        ...
                        if job_data.get("completed_at"):
                            ...
                            table.add_row("Completed", completed.strftime("%Y-%m-%d %H:%M:%S"))
                        ...
                        if metadata:
                            ...
                            if "pages_found" in metadata:
                                table.add_row("Pages Found", str(metadata["pages_found"]))
                            if "processed_pages" in metadata:
                                table.add_row("Processed Pages", str(metadata["processed_pages"]))
                            if "total_pages" in metadata:
                                table.add_row("Total Pages", str(metadata["total_pages"]))
                        ...
                        echo_error(f"Failed to get job status: {response.status_code} - {response.text}")
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                ...
                if response.status_code == 200:
                    ...
                    echo_error(f"Cannot cancel job: {result.get('detail', 'Unknown reason')}")
                else:
                    echo_error(f"Failed to cancel job: {response.status_code} - {response.text}")
...
@job.command()
@click.option("--days", default=7, help="Remove jobs older than this many days (default: 7)")
...
def cleanup(days, force):
    ...
    async def cleanup_jobs():
        try:
            if not force:
                if not click.confirm(f"Remove all completed/failed jobs older than {days} days?"):
                    echo_info("Cleanup aborted")
                    return
            
            async with httpx.AsyncClient() as client:
                response = await client.post(
                    get_api_url("admin/jobs/cleanup"),
                    json={"days": days},
                    timeout=30.0
                )
                ...
                if response.status_code == 200:
                    ...
                    echo_error(f"Failed to cleanup jobs: {response.status_code} - {response.text}")
